...
class MessageAdapter(MessageManager: MessageManager, function: (Message, View) -> Boolean) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (getItemViewType(position) === MISSATGE_D_USUARI) {
            repository.getMessage(position)?.let { (holder as MessageViewHolder).bind(it) };
        } else {
            repository.getMessage(position)?.let { (holder as MessageAltreViewHolder).bind(it) };
        }
    }
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        var message=repository.getMessage(position)
        if (message != null) {
